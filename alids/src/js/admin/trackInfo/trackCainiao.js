class TrackCainiao {
    ...
    parse() {
        ...
        let data = this.params.module[0];


        let status = [];

        let list = data.detailList.map((i)=>{

            return {
                text: i.desc,
                time: i.timeStr.split(' '),//"2019-06-22 16:45:51",
            }
        })


        return {
            params: [data.processInfo.progressPointList.map(i=> i.pointName)],
